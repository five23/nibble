...
var xgui = function(p) {
	...
	var pool = [];
	var mouseDownArray = [false, false, false, false, false, false, false, false, false, false, false];
	// 11 touch events, hopefully no one has more fingers than that...
	...
	function onMouseMove(event, isTouchEvent) {
		...
		for (var t = 0; t < loopNum; t++) {
			...
			if (o.name == "CheckBox" || o.name == "RadioButton" || o.name == "Button" || o.name == "ImageButton" || o.name == "Matrix")
				return;
	...
	function onMouseDown(event, isTouchEvent) {
		...
		var inputid = 0;
		if (isTouchEvent)
			inputid = event.changedTouches[0].identifier;
	...
	this.RangeSlider = function(p) {
		...
		this.width = p.width || 100, this.height = p.height || 10;
		this.min = p.min || 0;
	...
	this.RangeSlider.prototype.draw = function() {
		if (this.value1.v > this.max)
			this.value1.v = this.max;
		if (this.value1.v < this.min)
			this.value1.v = this.min;
		if (this.value2.v > this.max)
			this.value2.v = this.max;
		if (this.value2.v < this.min)
			this.value2.v = this.min;

		if (this.value1.v > this.value2.v)
			this.value1.v = this.value2.v;
		if (this.value2.v < this.value1.v)
			this.value2.v = this.value1.v;
	...
	this.DropDown = function(p) {
		...
		this.dropdown.style.background = "rgba(0,0,0,0)";
		//"transparent";
	...
	this.Stepper = function(p) {
		...
		this.width = p.width || 60, this.height = p.height || 10;
	...
	this.ColorPicker2.prototype.draw = function() {
		...
		if (this.open) {
			...
			var offset = 1 / 6;
			var index = 0;
			gradient.addColorStop(offset * index++, "rgb(255, 0, 0)");
			// red
			gradient.addColorStop(offset * index++, "rgb(255, 255, 0)");
			// yellow
			gradient.addColorStop(offset * index++, "rgb(0, 255, 0)");
			// green
			gradient.addColorStop(offset * index++, "rgb(0, 255, 255)");
			// cyan
			gradient.addColorStop(offset * index++, "rgb(0, 0, 255)");
			// blue
			gradient.addColorStop(offset * index++, "rgb(255, 0, 255)");
			// magenta
			gradient.addColorStop(offset * index++, "rgb(255, 0, 0)");
			// red
			...
			// white
			gradient.addColorStop(onepixel, "rgba(255, 255, 255, 1)");
			// white
			...
			// alpha
			gradient.addColorStop(1 - onepixel, "rgba(0, 0, 0, 1)");
			// black
	...
	this.CheckBox = function(p) {
		...
		this.width = this.fullWidth, this.height = p.height || 10;
		this.draw();
	}
